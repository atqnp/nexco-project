...
df_merged.to_csv('ryokin' + time_now + '.csv')
df_merg_comp.to_csv('comp_ryokin' + time_now + '.csv')
diff_ic.to_csv('error_ryokin' + time_now + '.csv')

with pd.ExcelWriter('ryokin' + time_now + '.xlsx') as writer:
    df_merged.to_excel(writer, sheet_name = 'まとめ')

with pd.ExcelWriter('ryokin' + time_now + '生データ.xlsx') as writer:
...
with pd.ExcelWriter('ryokin' + time_now + '種類分け.xlsx') as writer:
